...
def sqs_consumer(mock_handler):
  with patch("boto3.client") as mock_client:
    ...
    consumer.client = mock_client.return_value
    yield consumer
...
def test_poll_sqs_processes_messages(sqs_consumer, mock_handler):
  ...
  sqs_consumer.client.receive_message.side_effect = [
    {"Messages": messages},
  ...
  # Assert
  sqs_consumer.client.receive_message.assert_called_with(
  ...
  sqs_consumer.client.delete_message.assert_has_calls(
    [
...
def test_poll_sqs_handles_empty_response(sqs_consumer, mock_handler):
  ...
  sqs_consumer.client.delete_message.assert_not_called()
